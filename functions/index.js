...
//   // known issue. different arcana with exact name match show up as related.
//   if (nameKR) {
//     admin.database().ref(`/arcana/${event.params.arcanaID}/nameKR/${nameKR}`).set(true);
//   }
...
exports.migrateArcana = functions.database.ref('/loadFunction/{function}').onWrite(event => {

  const arcanaID = '-KydYE_y76ODICDk4Y0V';

  admin.database().ref(`/arcana/${arcanaID}`).once('value').then(snapshot => {
    ...
    admin.firestore().collection("arcana").doc(arcanaID).set(obj);
